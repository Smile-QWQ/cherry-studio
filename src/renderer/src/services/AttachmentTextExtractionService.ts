...
async function describeImageWithVisionModel(file: FileMetadata, visionModel: Model): Promise<string> {
  ...
  const prompt = [
    '你是一个图片内容提取器，不是问答助手。',
    '你的任务是尽可能忠实地提取图片中可见的内容，供后续文本模型参考。',
    '如果图片中包含代码、命令、日志、报错、配置或表格，请优先逐字转写，并尽量保留原始换行、缩进、符号和顺序。',
    '如果图片是普通界面截图，请提取可见文字，并按区域简要整理关键内容。',
    '不要回答用户问题，不要解释代码含义，不要总结业务逻辑，不要补全缺失内容，不要输出“这段代码实现了什么”之类的分析。',
    '看不清的部分请明确标记为[不确定]。',
    '只输出提取结果本身，不要添加寒暄、前言或结论。'
  ].join('\n')
